...
PPM PPM::vertical(){
    PPM newppm = *this;
    int total_pixels = mHeight * mWidth;
    int i, j, k;
    int height, width;
    height = getHeight();
    width = getWidth();
    float diff = mMaxColorValue * 0.10;
    for(i = 0; i < height ; i++){
        for (j = 0; j < width; j++){
            int prev_j = j-1;
            if (prev_j < 0){
            ...
            int pixel_r1 = getChannel(i, j, 0);
            int pixel_r2 = getChannel(i, prev_j, 0);
            int pixel_g1 = getChannel(i, j, 1);
            int pixel_g2 = getChannel(i, prev_j, 1);
            int pixel_b1 = getChannel(i, j, 2);
            int pixel_b2 = getChannel(i, prev_j, 2);
            if ((std::abs(pixel_r1 - pixel_r2) > diff) || (std::abs(pixel_g1 - pixel_g2) > diff) || (std::abs(pixel_b1 - pixel_b2) > diff) ){
                for ( k = 0 ; k < 3 ; k++){
                    newppm.setChannel(i, j, k, 255);
                    
                }
            }
            else{
                for (k = 0 ; k < 3 ; k++){
                    newppm.setChannel(i, j, k, 0);
                }
            }
        }
...
PPM PPM::horizontal(){
    ...
    for(i = 0; i < height ; i++){
        for (j = 0; j < width; j++){
            ...
            int pixel_r1 = getChannel(i, j, 0);
            int pixel_r2 = getChannel(prev_i, j, 0);
            int pixel_g1 = getChannel(i, j, 1);
            int pixel_g2 = getChannel(prev_i, j, 1);
            int pixel_b1 = getChannel(i, j, 2);
            int pixel_b2 = getChannel(prev_i, j, 2);
            if ((std::abs(pixel_r1 - pixel_r2) > diff) || (std::abs(pixel_g1 - pixel_g2) > diff) || (std::abs(pixel_b1 - pixel_b2) > diff) ){
                for ( k = 0 ; k < 3 ; k++){
                    newppm.setChannel(i, j, k, 255);
